    ...
    const candidates = dbCandidates.map((c: any) => {
      ...
      return {
        ...
        personalInfo: {
          ...
          languages: Array.isArray(c.languages) ? c.languages : (typeof c.languages === 'string' ? (() => { try { const p = JSON.parse(c.languages as string); return Array.isArray(p) ? p : []; } catch { return []; } })() : []),
          workExperience: Array.isArray(c.workExperience) ? c.workExperience : (typeof c.workExperience === 'string' ? (() => { try { const p = JSON.parse(c.workExperience as string); return Array.isArray(p) ? p : []; } catch { return []; } })() : []),
          skills: Array.isArray(c.skills) ? c.skills : (typeof c.skills === 'string' ? (() => { try { const p = JSON.parse(c.skills as string); return Array.isArray(p) ? p : []; } catch { return []; } })() : []),
...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const stringFields = [
      ...
      'labourId', 'status', 'agency', 'salary', 'price', 'visaOrContractNumber', 'sponsorName'
    ];
