...
// Safe array helpers — never crash even if field is null/undefined
const safeArr = <T,>(v: T[] | null | undefined): T[] => Array.isArray(v) ? v : [];
const safeJoin = (v: string[] | null | undefined, sep = ', ') => safeArr(v).join(sep);
const safeMap = <T, R>(v: T[] | null | undefined, fn: (item: T) => R): R[] => safeArr(v).map(fn);
const safeExpStr = (workExperience: any[] | null | undefined) =>
  safeMap(workExperience, e => `${e.experienceStatus} - ${e.country} (${e.yearsOfExperience})`).join(', ') || 'N/A';
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] p-8 text-[10px] leading-relaxed" style={{fontFamily:'Georgia, serif'}}>
      ...
      <div className="mt-4"><h3 className="font-bold text-gray-800 border-b border-gray-300 pb-1 mb-2">EDUCATION & LANGUAGES</h3>
        <p><strong>Education:</strong> {pi?.educationLevel}</p>
        <p><strong>Languages:</strong> {safeJoin(pi?.languages)}</p>
      </div>
      <div className="mt-4"><h3 className="font-bold text-gray-800 border-b border-gray-300 pb-1 mb-2">WORK EXPERIENCE</h3>
        <p>{safeExpStr(pi?.workExperience)}</p>
      </div>
      <div className="mt-4"><h3 className="font-bold text-gray-800 border-b border-gray-300 pb-1 mb-2">SKILLS</h3>
        <p>{safeJoin(pi?.skills, ' • ')}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] text-[10px] leading-relaxed overflow-hidden" style={{fontFamily:'Inter, sans-serif'}}>
      <div className="bg-primary text-white p-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold">{p?.givenNames} {p?.surname}</h1>
          <p className="text-indigo-100 mt-1">{p?.nationality} • {p?.gender}</p>
          <p className="text-indigo-200 text-[9px]">{pi?.phone} | {pi?.email}</p>
        </div>
      </div>
      <div className="p-6 space-y-4">
        ...
        <div><h3 className="text-xs font-bold text-primary uppercase mb-1">Languages</h3>
          <div className="flex flex-wrap gap-1">{safeArr(pi?.languages).map(l=><span key={l} className="px-2 py-0.5 bg-primary-50 text-primary rounded-full text-[9px]">{l}</span>)}</div>
        </div>
        <div><h3 className="text-xs font-bold text-primary uppercase mb-1">Skills</h3>
          <div className="flex flex-wrap gap-1">{safeArr(pi?.skills).map(s=><span key={s} className="px-2 py-0.5 bg-primary-50 text-primary rounded-full text-[9px]">{s}</span>)}</div>
        </div>
        <div><h3 className="text-xs font-bold text-primary uppercase mb-1">Experience</h3>
          <p className="text-gray-700">{safeExpStr(pi?.workExperience)}</p>
        </div>
        <div><h3 className="text-xs font-bold text-primary uppercase mb-1">Passport</h3>
          <p className="text-gray-700">No: {p?.passportNumber} | Issued: {formatDate(p?.dateOfIssue)} | Expires: {formatDate(p?.dateOfExpiry)}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] flex text-[10px] leading-relaxed overflow-hidden" style={{fontFamily:'Inter, sans-serif'}}>
      <div className="w-1/3 bg-gray-900 text-white p-5 flex flex-col items-center">
        ...
        <h2 className="text-sm font-bold text-center">{p?.givenNames}</h2>
        <p className="text-gray-400 text-center">{p?.surname}</p>
        <div className="w-full mt-4 space-y-3">
          ...
          <div><p className="text-[8px] text-gray-500 uppercase">Languages</p>
            {safeArr(pi?.languages).map(l=><p key={l} className="text-gray-300">{l}</p>)}
          </div>
        </div>
      ...
      <div className="flex-1 p-6 space-y-4">
        ...
        <div><h3 className="text-xs font-bold text-gray-800 mb-2">PERSONAL DETAILS</h3>
          <div className="grid grid-cols-2 gap-2">
            <p><strong>DOB:</strong> {formatDate(p?.dateOfBirth)}</p><p><strong>Gender:</strong> {p?.gender}</p>
            <p><strong>Nationality:</strong> {p?.nationality}</p><p><strong>Marital:</strong> {pi?.maritalStatus}</p>
            <p><strong>Religion:</strong> {pi?.religion}</p><p><strong>Blood Type:</strong> {pi?.bloodType}</p>
          </div>
        </div>
        <div><h3 className="text-xs font-bold text-gray-800 mb-2">EDUCATION</h3><p>{pi?.educationLevel}</p></div>
        <div><h3 className="text-xs font-bold text-gray-800 mb-2">EXPERIENCE</h3><p>{safeExpStr(pi?.workExperience)}</p></div>
        <div><h3 className="text-xs font-bold text-gray-800 mb-2">SKILLS</h3>
          <div className="flex flex-wrap gap-1">{safeArr(pi?.skills).map(s=><span key={s} className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-[9px]">{s}</span>)}</div>
        </div>
        <div><h3 className="text-xs font-bold text-gray-800 mb-2">PASSPORT</h3>
          <p>No: {p?.passportNumber} | Valid: {formatDate(p?.dateOfIssue)} - {formatDate(p?.dateOfExpiry)}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white w-full aspect-[210/297] p-10 text-[10px] leading-loose" style={{fontFamily:'Inter, sans-serif'}}>
      <div className="flex items-center gap-5 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-light text-gray-900 tracking-wide">{p?.givenNames} {p?.surname}</h1>
          <p className="text-gray-400 mt-1">{pi?.email} • {pi?.phone}</p>
        </div>
      </div>
      <div className="h-px bg-gray-200 mb-6" />
      <div className="space-y-5">
        {[
          {t:'Personal',   c:`${formatDate(p?.dateOfBirth)} • ${p?.gender ?? ''} • ${p?.nationality ?? ''} • ${pi?.maritalStatus ?? ''} • ${pi?.religion ?? ''}`},
          {t:'Education',  c: pi?.educationLevel ?? ''},
          {t:'Languages',  c: safeJoin(pi?.languages)},
          {t:'Experience', c: safeExpStr(pi?.workExperience)},
          {t:'Skills',     c: safeJoin(pi?.skills)},
          {t:'Passport',   c:`${p?.passportNumber ?? ''} — Valid until ${formatDate(p?.dateOfExpiry)}`},
        ].map(s=>(
          <div key={s.t}>
            <h3 className="text-[9px] font-semibold uppercase tracking-widest text-gray-400 mb-1">{s.t}</h3>
            <p className="text-gray-700">{s.c}</p>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="grid grid-cols-2 gap-5">
          <div className="space-y-3">
            <div><h3 className="text-xs font-bold text-amber-700 border-b border-amber-200 pb-1 mb-2">Personal Information</h3>
              <p>Date of Birth: {formatDate(p?.dateOfBirth)}</p><p>Gender: {p?.gender}</p>
              <p>Marital Status: {pi?.maritalStatus}</p><p>Religion: {pi?.religion}</p>
              <p>Children: {pi?.numberOfChildren ?? ''}</p>
            </div>
            <div><h3 className="text-xs font-bold text-amber-700 border-b border-amber-200 pb-1 mb-2">Contact</h3>
              <p>{pi?.phone}</p><p>{pi?.email}</p><p>{pi?.city}, {pi?.country}</p>
            </div>
          </div>
          <div className="space-y-3">
            <div><h3 className="text-xs font-bold text-amber-700 border-b border-amber-200 pb-1 mb-2">Languages & Skills</h3>
              <p><em>Languages:</em> {safeJoin(pi?.languages)}</p>
              <p><em>Skills:</em> {safeJoin(pi?.skills)}</p>
            </div>
            <div><h3 className="text-xs font-bold text-amber-700 border-b border-amber-200 pb-1 mb-2">Experience</h3>
              <p>{safeExpStr(pi?.workExperience)}</p>
            </div>
            <div><h3 className="text-xs font-bold text-amber-700 border-b border-amber-200 pb-1 mb-2">Passport Details</h3>
              <p>No: {p?.passportNumber}</p><p>Issued: {formatDate(p?.dateOfIssue)}</p>
              <p>Expires: {formatDate(p?.dateOfExpiry)}</p>
            </div>
          </div>
        </div>
      </div>
